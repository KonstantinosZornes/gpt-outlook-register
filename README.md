...
- 📱 **SMS 接码集成（解决 add-phone 拿不到 refresh_token 的问题）**
  - 新增 **HeroSMS** 平台支持（sms-activate 协议系，与 SmsBower 共用同一套代码）
  - 国家列表改为**动态获取**（查询平台 API 实际库存，切换平台自动刷新）
  - 「📱 接码配置」Tab 一键启用 SmsBower / HeroSMS / 5sim 平台
...
| `sms_provider.py` | ~1900 | **SMS 接码 provider**：BaseSmsProvider + SmsBower / HeroSMS / 5sim + PhoneCallbackController |
...
| `OPENAI_PHONE_NUMBER` | - | 手动 add-phone 时的号码（用 `,` 分隔多个候选）。**未启用 SMS 接码时用** |
| `OPENAI_PHONE_OTP` | - | 静态 OTP 验证码（调试用，6 位数字） |
| `OPENAI_PHONE_OTP_CMD` | - | 动态获取手机 OTP 的 shell 命令（stdout 含 6 位数字即命中） |
| `OPENAI_PHONE_OTP_TIMEOUT` | `180` | add-phone 等 SMS 超时（秒） |
...
| **5sim** | 5sim.net | REST + Bearer Token；国家用 slug（如 `thailand`）；Service=`openai` |
...
1. 注册平台账号 + 充值 + 拿 API Key
2. WebUI →「📱 接码配置」→ 启用 + 填 API Key + 选国家（默认 52 = Thailand）→ 保存
3. 点「测试余额」确认连通
4. 正常触发注册，命中 add-phone 时会自动租号，日志里能看到 `📱 准备租号` / `✅ 已租到号码` / `⏳ 等待 SMS 验证码` / `🎉 已标记号码成功完成`
...
| `sms_provider` | herosms / smsbower / 5sim |
| `sms_country` | 国家：sms-activate 系用数字 ID（`52`=泰国）；5sim 用 slug（`thailand`） |
| `sms_service` | OpenAI：sms-activate 系=`dr`；5sim=`openai` |
| `sms_max_price` | 单号最高价格（平台货币单位） |
| `sms_reuse_phone` | 同号复用（SmsBower / HeroSMS 主推；5sim 下单传 reuse=1） |
| `sms_phone_success_max` | 同号最多复用 N 次 |
| `sms_auto_country` | 自动选最优国家（按价格 + 库存） |
...
- **5sim**：Bearer Token 鉴权；国家用英文 slug（`thailand`/`england`…，也兼容填 `52` 自动映射）；Service 填 `openai`（填 `dr` 会自动映射）；拒号走 ban，成功走 finish。
